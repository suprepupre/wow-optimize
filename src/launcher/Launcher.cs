    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "No Client Patches (diagnostic)", new SettingItem("General", "NoClientPatches", false, null, "Writes nothing at all into WoW.exe, which turns every optimization off. The disconnect watch, the crash reporter and the frame timing keep running because they live outside the client. On WoWCircle this is the difference between being dropped every session and not: two players ran it and the disconnects stopped, and both got dropped again with it off. It is the trade, not a fix - you keep your connection and lose the performance work. Every report line says it is on, so such a log is never mistaken for a normal one.", true) },
                { "Flight Recorder (mark a moment)", new SettingItem("General", "FlightRecorder", true, null, "Keeps the last 512 frames in memory and writes 240 of them to the log the moment you press Scroll Lock. Use it when you can see something going wrong: press the key right then, and the log carries that second frame by frame instead of the ten-second averages everything else reports. It records shadow map redraws and buffer swaps, and Lua chunks compiled, per frame. Nothing is written until you press the key. A key rather than a slash command because it also works on a loading screen and at character select, where no addon is running. Change the key with FlightRecorderKey in wow_opt.ini if Scroll Lock is taken. On by default: it writes nothing until a mark, and it marks itself for a disconnect, a freeze and a bad SavedVariables filename, which are the three things nobody can press a key for.") },
                { "A/B Test a Feature", new SettingItem("General", "AbTest", false, null, "Turns one feature on and off in alternating stints while you play, and reports the frame times of each half separately. Between two sessions nothing is held still - a different zone, a different raid, a different evening - which is why no optimization here has ever had a measured gain. Alternating inside one session puts the same zone, the same addons and the same machine in both halves so they cancel. Ticking this on its own takes every feature that can be tested in turn, so one long session measures all of them; only the feature being measured is alternated, so none of them affects another. It can only measure features you have switched on - a feature registers with it when it installs - so on a default install there are two, and the report says so rather than going quiet. Tick the ones in Experimental you want an answer about before you start. To spend the whole session on one instead, put its name in wow_opt.ini as AbTestSubject=LayoutRelinkFast and turn that feature on as well - this decides when it works, not whether it installed. Play for at least ten minutes, and much longer when it is rotating. The report names the features it will answer to if you get the name wrong, and says per feature when there are too few stints to trust.", true) },
                ...
                { "Event Coalescing", new SettingItem("Combat_Net", "EventCoalescer", false, null, "When the game raises the same interface event several times inside one frame, this passes it on once. It is listed here because the DLL has always read this setting while the launcher never offered it, so the only way to change it was to edit the ini by hand. Leave it off unless you are testing it: it drops repeats of events that do not carry which aura or spell changed, so buff icons and cooldown countdowns can stop updating. That defect is open and is why the default is off.", true) },
