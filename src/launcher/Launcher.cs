    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Addon CPU Profiler", new SettingItem("UI_Lua", "AddonProfiler", false, null, "Answers \"which of my addons is eating the frame rate\". The client has a script profiler built in that nothing in the interface ever switches on; this switches it on and writes a ranked table to your log every minute - each addon, its milliseconds, and its share. Off by default because the client's profiler is not free. Turn it on for one session when the game stutters, send the log, turn it off. If the first report shows everything at zero, type /reload once: the client only accounts for scripts it compiled after profiling started.", true) },
