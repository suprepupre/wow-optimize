    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Multithreaded Nameplates", new SettingItem("Combat_Net", "NameplateMT", true, null, "Moves nameplate work off the main thread, which matters most where there are a lot of units on screen - raids, and Dalaran. On by default because it ran on every install until 3.18.1, when it was given a switch that defaulted to off and quietly took it away from everyone; a tester reported that as the game getting worse in exactly those places. It uses worker threads, so it is skipped automatically under Wine and Rosetta. Turn it off if you suspect threading trouble.") },
                { "Async SavedVariables Writer", new SettingItem("Combat_Net", "SavedVarsAsync", true, null, "Writes your SavedVariables on a background thread instead of stalling the game while it saves. Same history as Multithreaded Nameplates: it ran everywhere until 3.18.1 gave it an off-by-default switch. Skipped automatically under Wine and Rosetta.") },
