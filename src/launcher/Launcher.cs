    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Fast UI Layout Relink", new SettingItem("UI_Lua", "LayoutRelinkFast", true, null, "The biggest single thing the client spends CPU on while playing: 9% of all executing main-thread time in a 28-minute profile, more than double anything else. Every time a frame is re-laid out, the client walks every frame in the interface and checks nine anchor points on each, looking for one anchored to it. It already keeps a list of exactly that on every frame, so for the frames nothing anchors to - which is most of them - the walk is skipped entirely. The result is identical; it is the same code taking the same path, just without searching for something that cannot be there. On startup and for several thousand relinks it runs the client's own search alongside and turns itself off if they ever disagree.") },
