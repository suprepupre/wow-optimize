    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Object Lookup Chain", new SettingItem("General", "ObjMgrFindFast", false, null, "When the game looks up a creature, player or item by its id, it walks a chain of entries. For every step of that chain it re-reads the table header and recomputes where the next link lives, even though none of that can change during one lookup. This works it out once instead. It runs alongside the game's own routine at first and compares every answer; a single difference switches it off for the session and says so in the log. EXPERIMENTAL.", true) },
