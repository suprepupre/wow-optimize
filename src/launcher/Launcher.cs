    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Steadier Shadows (flicker fix)", new SettingItem("Graphics_Sound", "ShadowCascadeHold", false, null, "Below the highest shadow setting the game does not redraw shadows every frame. It keeps three shadow maps around a remembered point, refreshes a third of one per frame, and picks a new point whenever you move away from the old one - and picking a new point restarts the refresh, so two thirds of the shadow is briefly wrong. Measured on a tester's session: the nearest map picks a new point every two yards, which while running is about three times a second, and that is the flicker people see on buildings as they run past. This lets the point stay put for longer, so the shadow stays complete for longer between refreshes. It will never let the point fall so far behind that you walk outside the area the shadow covers - it reads how large that area is and limits itself. Turn it off if shadows near you look like they are lagging. EXPERIMENTAL, and it needs the shadow probe switched on as well.", true) },
