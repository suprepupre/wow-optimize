    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "No Client Patches (diagnostic)", new SettingItem("General", "NoClientPatches", false, null, "Diagnostic only: writes nothing at all into WoW.exe, which turns every optimization off. The disconnect watch, the crash reporter and the frame timing keep running because they live outside the client. Turn this on for one session if you are being disconnected, so the log can say whether the drops follow the patches or happen without them. Every report line says it is on, so such a log is never mistaken for a normal one.", true) },
