    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "A/B Test a Feature", new SettingItem("General", "AbTest", false, null, "Turns one feature on and off in alternating stints while you play, and reports the frame times of each half separately. Between two sessions nothing is held still - a different zone, a different raid, a different evening - which is why no optimization here has ever had a measured gain. Alternating inside one session puts the same zone, the same addons and the same machine in both halves so they cancel. Set AbTestSubject in wow_opt.ini to the feature you want tested, for example AbTestSubject=LayoutRelinkFast, and turn that feature on as well - this decides when it works, not whether it installed. Or set AbTestSubject=all and it takes every feature that can be tested in turn, so one long session measures all of them instead of one; only the feature being measured is alternated, so none of them affects another. Play for at least ten minutes, and much longer for all. The report names the features it will answer to if you get the name wrong, and says per feature when there are too few stints to trust.", true) },
