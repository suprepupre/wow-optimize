    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Shadow Flicker Experiment", new SettingItem("Graphics_Sound", "ShadowBufferAlternate", false, null, "An experiment, not a fix, and it can go wrong. The shadow flicker below the highest Shadow Quality is a bug in the game itself. Comparing a broken session against a working one narrowed it to one thing: at the highest setting the game alternates which buffer it draws shadows into every frame, and at every lower setting it reuses the same one - which is a well-known way to get exactly this flicker on some drivers. This changes two bytes so it alternates at every setting. It might fix it, it might do nothing, and it might misbehave, because I have not been able to prove the game allocates more than one buffer at the lower settings. Only turn it on if you are testing this for me, and turn it off if anything looks wrong.", true) },
