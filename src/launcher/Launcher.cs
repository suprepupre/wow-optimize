    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Lua Pool Shortcuts", new SettingItem("UI_Lua", "LuaPoolFast", false, null, "The game keeps its own pool of memory for the interface scripting language, carved into chunks. Every time it hands a block back, it has to work out which chunk that block came from, and it does that by checking every chunk in turn, following a pointer to each one before it can even compare. This remembers the last few chunks along with their boundaries, so the usual answer is a couple of comparisons instead of a walk through scattered memory. The block is always re-checked against the chunk's own record before anything is written, so a stale entry costs a little time and can never put memory in the wrong place. Handing memory out has the same problem and worse: the game looks for the first chunk with room starting from the very first one every single time, so once the early chunks fill up they get walked past on every allocation for the rest of the session. This remembers how far down the empty ones reach, and still checks the rest before letting the pool grow, so it hands back the same block the game would have. Two testers' freeze reports have pointed at this exact code. EXPERIMENTAL.", true) },
